...
router.post("/login", async (req, res) => {
    try {
        ...
        if(!loginCustomer) return res.status(400).send("Email or password incorrect");
...
router.post("/check", async (req, res) => { // check user email and id
    try {
        ...
        if(!email || !idNumber) return res.status(400).send("missing info"); 
        const isExist = await authLogic.checkIdAndEmailAsync(email, idNumber);
        if(isExist) return res.status(400).send("An email or ID card already exists in the system");
